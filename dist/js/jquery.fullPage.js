...
(function($) {
	$.fn.fullpage = function(options) {
		...
		var scrtop = false;
		var isOnDiv = false;
		$(".boardz").mouseenter(function(){isOnDiv=true;});
		...
		function MouseWheelHandler() {
				return function(e) {
						...
						if (!isMoving) { //if theres any #
							//scrolling down?
							if (delta < 0) {
								if((bottom || !isOnDiv) && document.getElementById('myModal').style.display != "block"){
									if(bottom){
										setTimeout( function(){ 
											$('#flux' + section).scrollTop(0);
										}  , 1000 );
										bottom = false
										scrtop = true;
									}
							...
							//scrolling up?
								...
								if((scrtop || !isOnDiv) && document.getElementById('myModal').style.display != "block"){
									if(scrtop){
										scrtop = true;
										bottom = false;
									}
					...
					$('#flux' + section).on('scroll', function(){
		...
		$.fn.fullpage.moveSlideDown = function (){
			...
			console.log(lastScrolledDestiny);
		}
		...
		function scrollPage(element) {
			...
			switch(anchorLink){
				case 'firstPage' : 	active_section = 1;
							break;
				case 'secondPage' : active_section = 2;
							break;
				case '3rdPage' : 	active_section = 3;
							break;
				case '4thpage' : 	active_section = 4;
							break;
				case 'lastPage' : 	active_section = 5;
							break;
				case 'contact' : 	active_section = 6;
							break;
			}
			//flag to avoid callingn `scrollPage()` twice in case of using anchor links
			lastScrolledDestiny = anchorLink;
			console.log($('#flux' + active_section).height());
			if($('#flux' + active_section).height() > $('#flux' + active_section).find(".boardz").height())
				bottom = true;
			else
				bottom = false;
